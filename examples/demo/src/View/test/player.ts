...
export class Player {
    ...
    skipToTabShown(a) {
        ...
        if (b._isPlayingNormal() && b._config.skipProlongedInactivity && !b._skippingToTabShown) {
            ...
            var lastRenderedActivity = b._render.lastRenderedActivity,
                endTime = b._pauseAt || Number.POSITIVE_INFINITY,
                e = b._activities.getSessionLength(),
                f = Math.min(a + b._config.tabHiddenMessageTime, e);

            if (a < endTime && endTime <= f) {
                var activities = b._activities.getIteratorBetween(c, endTime);
            ...
            b._playback.replay(function () {
                ...
                if (!b._render.isTabHidden) return b.play(f);
    ...
    _createNormalPlayback(time: number, endTime: number, activities): NormalPlayback {
        var normalPlayback = new NormalPlayback(time, endTime, activities, this._render, this._config);
